...
class SupabaseManager:
    ...
    def refresh_session(self, refresh_token):
        ...
        Returns the new session object if successful, None otherwise.
        """
        if not self.supabase: return None
        try:
            # print(f"🔄 Attempting Session Refresh...")
            res = self.supabase.auth.refresh_session(refresh_token)
            if res and res.session:
                # print(f"✅ Session Refreshed Successfully for {res.user.email}")
                return res
            return None
        except Exception as e:
