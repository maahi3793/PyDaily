        ...
        with st.form("add_contact"):
            ...
            if submitted:
                if name and email:
                    ...
                    success, msg = db.admin_create_student(email, name, temp_pass)
                    if success:
                ...
                if st.button("Update Password Now", type="secondary"):
                    if len(new_pass) < 6:
                        ...
                        with st.spinner("Updating in Database..."):
                            success, msg = db.admin_update_password(selected_email, new_pass)
                            if success:
            ...
            with st.expander("🛠️ Advanced Progress Edit", expanded=False):
                ...
                with st.form("manual_edit"):
                    ...
                    if st.form_submit_button("Force Update"):
                        # Returns success, msg
                        success, msg = db.admin_update_student_progress(selected_email, day=new_day, status=new_status)
